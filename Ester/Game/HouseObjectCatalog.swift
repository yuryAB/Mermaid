...
enum HouseObjectCatalog {
    ...
    static let allDefinitions: [HouseObjectDefinition] = floorFurnitureSpecs.map { spec in
    ...
    static func definition(_ definition: HouseObjectDefinition,
        ...
        guard let spec = floorFurnitureSpecs.first(where: { $0.id == definition.id }) else {
            return definition
        }
        let width = roomSize.width * spec.roomWidthFraction
        let height = width * (spec.defaultSize.height / spec.defaultSize.width)
